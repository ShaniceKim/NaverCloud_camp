-- 회원
DROP TABLE IF EXISTS MY_SCHEMA.myapp_member RESTRICT;

-- 게시글
DROP TABLE IF EXISTS MY_SCHEMA.myapp_board RESTRICT;

-- 게시판유형
DROP TABLE IF EXISTS MY_SCHEMA.myapp_board_category RESTRICT;

-- 게시글첨부파일
DROP TABLE IF EXISTS MY_SCHEMA.myapp_board_file RESTRICT;

-- 회원
CREATE TABLE MY_SCHEMA.myapp_member (
	...
	COL          VARCHAR(255) NULL     COMMENT '사진' -- 사진
)
COMMENT '회원';

-- 회원
ALTER TABLE MY_SCHEMA.myapp_member
...
-- 게시글
CREATE TABLE MY_SCHEMA.myapp_board (
...
-- 게시글
ALTER TABLE MY_SCHEMA.myapp_board
...
-- 게시판유형
CREATE TABLE MY_SCHEMA.myapp_board_category (
...
-- 게시판유형
ALTER TABLE MY_SCHEMA.myapp_board_category
...
CREATE UNIQUE INDEX UIX_myapp_board_category
	ON MY_SCHEMA.myapp_board_category ( -- 게시판유형
		name ASC -- 게시판이름
	);
...
-- 게시글첨부파일
CREATE TABLE MY_SCHEMA.myapp_board_file (
...
-- 게시글첨부파일
ALTER TABLE MY_SCHEMA.myapp_board_file
...
ALTER TABLE MY_SCHEMA.myapp_board
	...
	REFERENCES MY_SCHEMA.myapp_member ( -- 회원
	member_no -- 번호
	);
...
ALTER TABLE MY_SCHEMA.myapp_board
	...
	REFERENCES MY_SCHEMA.myapp_board_category ( -- 게시판유형
	board_category_no -- 번호
	);
...
ALTER TABLE MY_SCHEMA.myapp_board_file
	...
	REFERENCES MY_SCHEMA.myapp_board ( -- 게시글
	board_no -- 번호
	);